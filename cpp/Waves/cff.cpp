...
CFF::CFF(CHistory *history, bool fun): CHistoryWorker(history)
{
	this->fun = fun;
}
...
void CFF::end()
{
	if(asInt)
	{
		str = QObject::tr("ЦБПФ{%1}").arg(history->str);
		kiss_fft_cfg cfg = kiss_fft_alloc(history->points.count(), fun, 0, 0);
	
		int i;
		kiss_fft_cpx *in = new kiss_fft_cpx[history->points.count()], *out = new kiss_fft_cpx[history->points.count()];
		for(i = 0; i < history->points.count(); i++)
		{
			in[i] = kiss_fft_cpx(round(history->points[i].real()), round(history->points[i].imag()));
		}
		
		kiss_fft(cfg, in, out);
		
		points.reserve(history->points.count());
		for(i = 0; i < history->points.size(); i++)
		{
			points.append(compct(out[i].real(), out[i].imag()));
		}
	...
	{
		str = QObject::tr("БПФ{%1}").arg(history->str);
		int i;
		ShortComplex *array = new ShortComplex[history->points.count()];
		for(i = 0; i < history->points.count(); i++)
		{
			array[i] = history->points[i];
		}
		universal_fft(array, history->points.count(), fun);
		
		points.reserve(history->points.count());
		
		for(i = 0; i < history->points.count(); i++)
		{
			points.append(compct(array[i].real() / history->points.count(), array[i].imag() / history->points.count()));
		}
...
static void fft_step(ShortComplex *x, unsigned int T, bool complement, const ShortComplex *Wstore)
{
    ...
    for(N= 2, Skew= Nmax, Step= 1; N <= Nmax; N += N, Skew >>= 1, Step++)
    {
        N2= N + N;
        Nd2= (N >> 1);
        for(Warray= (ct*)Wstore, k= 0; k < Nd2; k++, Warray += Skew)
        {
            Wre= *Warray;
            Wim= cmul*Warray[1];
            for(ct *x1re= arr + (k + k); x1re < arrEnd; x1re+= N2)
            {
                ct *x1im= x1re + 1;
                ct *x2re= x1re + N;
                ct *x2im= x2re + 1;
...
void universal_fft(ShortComplex *x, int N, bool complement)
{
    ...
    int T;
    T= (int)floor(log((ct)N) / log(2.0) + 0.5);
    if (1 << T == N)
    {
